...
//	x1------x7---------x2
//		x6			x4
Composition::Composition(int tx1, int ty1, int tx2, int ty2):arrow(tx1,ty1,tx2,ty2)
{
	type=0;
...
void Composition::ondraw(CDC *pDC)
{
...
int Composition::onpressLB(int x, int y)
{
	CRgn rgn;
	CPoint p[3],t[4],l[4];
	p[0].x=x2;p[0].y=y2;
...
void Composition::onmove(int cx, int cy)
{
	if(state==1)
	{
		x7+=cx;
